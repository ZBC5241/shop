...
/* 逐人下钻：展示该员工在「渠道挂账」表格里的任务/完成/缺口/完成率（严格按表格公式） */
function personDetailHTML(name){
  const list = (DATA.qudao && DATA.qudao.people) || [];
  const p = list.find(x => x.name === name);
  if(!p) return '<div class="det-empty">该员工暂无渠道挂账数据</div>';
  const has = isNum(p.task) && p.task !== 0;
  const rows = [
    ['任务目标', has ? moneyShort(p.task) : '—'],
    ['本期完成', moneyShort(p.done)],
    ['完成率',   has ? pct(p.rate,1) : '无任务'],
    ['缺口',     has ? (isNum(p.gap) && p.gap < 0 ? '还差 ' + moneyShort(Math.abs(p.gap)) : '超出 ' + moneyShort(p.gap||0)) : '—'],
  ];
  let h = '<div class="det-list">';
  rows.forEach(rw => {
    h += '<div class="det-item">'
      + '<div class="det-top"><span class="det-name">' + esc(rw[0]) + '</span>'
      + '<span class="det-date num">' + rw[1] + '</span></div></div>';
  });
  h += '</div>';
  return h;
}
